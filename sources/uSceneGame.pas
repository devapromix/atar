...
uses Classes, Graphics, uScene, uMap, uTile;
...
begin
  ...
  begin
    ...
    if (I < 0) then I := 0;
    Game.Load(I);
    if B then Go(tlAltNextDungeon)
      else Go(tlNextDungeon);
    Scenes.Render;
  end;
end;
...
begin
    ...
    begin
      if (Length(Creatures.Enemy) > 0) then
        for I := 0 to High(Creatures.Enemy) do
          if (Creatures.PC.Look.X = Creatures.Enemy[I].Pos.X)
            and (Creatures.PC.Look.Y = Creatures.Enemy[I].Pos.Y) then
          begin
            Graph.Messagebar.Add('#r' + GetCreatureLang(Creatures.Enemy[I].Name) +
  ...
  case T of
    tlLockedDoor     : Graph.Messagebar.Add(GetLang(58));
    tlClosedDoor     : Graph.Messagebar.Add(GetLang(59));
    tlPrevDungeon    : Graph.Messagebar.Add(Format(GetLang(83), [GetMapLang(Map.Info.PrevMap)]));
    tlNextDungeon    : Graph.Messagebar.Add(Format(GetLang(84), [GetMapLang(Map.Info.NextMap)]));
    tlAltNextDungeon : Graph.Messagebar.Add(Format(GetLang(84), [GetMapLang(Map.Info.AltNextMap)]));
    tlEmptyShrine    : Graph.Messagebar.Add(GetLang(85));
    tlLifeShrine     : Graph.Messagebar.Add(GetLang(86));
    tlManaShrine     : Graph.Messagebar.Add(GetLang(86));
    tlMegaShrine     : Graph.Messagebar.Add(GetLang(86));
    tlClosedBarrel   : Graph.Messagebar.Add(GetLang(57));
    tlClosedWoodChest: Graph.Messagebar.Add(GetLang(87));
    tlLockedWoodChest: Graph.Messagebar.Add(GetLang(88));
    tlLockedBestChest: Graph.Messagebar.Add(GetLang(88));
  end;
  // Item
  if (Length(Items.Item) > 0) then
    for I := 0 to High(Items.Item) do
      if (Creatures.PC.Pos.X = Items.Item[I].Pos.X) and (Creatures.PC.Pos.Y = Items.Item[I].Pos.Y) then
      begin
        Graph.Messagebar.Clear;
        ...
        if (Items.CellItemsCount(Creatures.PC.Pos.X, Creatures.PC.Pos.Y) > 1) then J := 0 else J := 1;
        case T of
  ...
  if F then Exit;
  case T of
    tlOpenBarrel : Graph.Messagebar.Add(GetLang(56));
    tlOpenWoodChest, tlOpenBestChest:
...
var
  ...
  begin
    Map.Cell[Creatures.PC.Pos.Y + AY][Creatures.PC.Pos.X + AX].Tile := tlOpenDoor;
    Log.Add(GetLang(46));
  end;
  ...
  begin
    ...
    begin    
      if (GetDist(Creatures.PC.Pos.X, Creatures.PC.Pos.Y,
        ...
        > Creatures.PC.Prop.Distance) then Exit;
    ...
    // Move
    if Creatures.PC.FreeCell(Creatures.PC.Pos.X + X,
      Creatures.PC.Pos.Y + Y) then Creatures.PC.Move(X, Y) else
    case Map.Cell[Creatures.PC.Pos.Y + Y][Creatures.PC.Pos.X + X].Tile of
      ...
      tlHiddenDoor:
        begin
          case Rand(1, 9) of
            1..4: T := tlClosedDoor;
            5..6: T := tlClosedDoorWithFireTrap;
            7..8: T := tlClosedDoorWithLightningTrap;
            9   : T := tlLockedDoor;
          end;
          Map.Cell[Creatures.PC.Pos.Y + Y][Creatures.PC.Pos.X + X].Tile := T;
          Log.Add(GetLang(43));
        end;
    end;
    Creatures.Move;
    ...
    13: case CursorMode of
          ...
          begin
            ...
            if not Creatures.Enemy[I].Life.IsMin
              and (Look.X = Creatures.Enemy[I].Pos.X)
              and (Look.Y = Creatures.Enemy[I].Pos.Y) then
            begin
              Ranged(I);
              Graph.Messagebar.Clear;  
              Exit;
...
//          Game.Load(1);
    ...
    // Move //
    Ord('W'),
    12,101,53: Creatures.PC.Wait;
    35,97,49 : if ssShift in Shift then Run(-1,1) else Move(-1,1);
    40,98,50 : if ssShift in Shift then Run(0,1) else Move(0,1);
    34,99,51 : if ssShift in Shift then Run(1,1) else Move(1,1);
    37,100,52: if ssShift in Shift then Run(-1,0) else Move(-1,0);
    39,102,54: if ssShift in Shift then Run(1,0) else Move(1,0);
    36,103,55: if ssShift in Shift then Run(-1,-1) else Move(-1,-1);
    38,104,56: if ssShift in Shift then Run(0,-1) else Move(0,-1);
    33,105,57: if ssShift in Shift then Run(1,-1) else Move(1,-1);
    // Use object //
    32, ord('U'):
        begin
          case Map.Cell[Creatures.PC.Pos.Y][Creatures.PC.Pos.X].Tile of
            tlLifeShrine:
              begin
                Graph.Messagebar.Clear;
                with TAnimNumber.Create(Creatures.PC.Life.Max - Creatures.PC.Life.Cur) do Free;
                Creatures.PC.Life.SetToMax;
                Map.Cell[Creatures.PC.Pos.Y][Creatures.PC.Pos.X].Tile := tlEmptyShrine;
            ...
            tlManaShrine:
              begin
                Graph.Messagebar.Clear;
                with TAnimNumber.Create(Creatures.PC.Mana.Max - Creatures.PC.Mana.Cur) do Free;
      ...
      ord('H'):
        if ParamDebug then GoToPrevMap;
      ord('J'):
    ...
    on E: Exception do Error.Add('SceneGame.KeyDown (#' + IntToStr(Key) + ')', E.Message);
  end;
end;
...
begin
  ...
  Map.Render;
  TT := GetTickCount - TT;
  if (MaxTT < TT) then MaxTT := TT;
  if ParamDebug then
    with Graph.Surface.Canvas do
    begin
      Brush.Style := bsClear;     
      Font.Color := cWhiteYel;
      TextOut(0, Graph.CharHeight,
        IntToStr(TT) + '(' + IntToStr(MaxTT) + ')');
      TextOut(0, Graph.CharHeight * 2,
        Format('X%d:Y%d', [Creatures.PC.Pos.X, Creatures.PC.Pos.Y]));
      TextOut(0, Graph.CharHeight * 3,
        Format('LX%d:LY%d', [Creatures.PC.Look.X, Creatures.PC.Look.Y]));
    end;
