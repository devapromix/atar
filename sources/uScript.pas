...
uses SysUtils, uUtils, Types, Classes;
...
uses uBox, uMap, uCreatures, uError, uItem;
...
begin
  ...
  begin
    FL[I] := Trim(FL[I]);
    if (FL[I] = '') or (FL[I][1] = ';') or (Pos(' ', FL[I]) < 1) then Continue;
    R := Explode(' ', FL[I]);
    for J := 0 to High(R) do R[J] := Trim(R[J]);
    // MapLevel
    try
    ...
    try
      ...
      begin
        ...
        if (Length(R) >= 5) then Enemy[High(Enemy)].Life.SetCur(StrToInt(R[4]));
        if (Length(R) >= 6) then Enemy[High(Enemy)].Mana.SetCur(StrToInt(R[5]));
      end;
    except
    ...
    try
      ...
      begin
        Add(StrToInt(R[1]), StrToInt(R[2]), R[3]);
        if (Length(R) >= 5) then Item[High(Item)].Prop.Tough := StrToInt(R[4]);
        if (Length(R) >= 6) then Item[High(Item)].Count := StrToInt(R[5]);
      end;
    except
      ShowError('AddItem', I);
    end;
...
begin
  Box(Format('Operator <%s> except error!' + #13 +
             'Script file: "%s"' + #13 +
             'Line: %d' + #13 +
             'Code: %s', [Operator, FFileName, Line, FL[Line]]),
             'Script Error!');
  Halt;
end;
