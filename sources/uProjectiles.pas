...
uses Graphics, uCreature, uCreatures;
...
uses Windows, SysUtils, Math, Types, uGraph, uUtils, uColor;

{ TProjectile }

constructor TProjectile.Create(ACreature: TCreature; FromX, FromY, ToX, ToY: Integer);
var
  ...
  begin
    ...
    begin
      if (FromY > ToY) then
        LoadBitmap(Z * 10 + 0) else LoadBitmap(Z * 10 + 4);
      Exit;
    end;
    if (FromY = ToY) then
    begin
      if (FromX > ToX) then
        LoadBitmap(Z * 10 + 6) else LoadBitmap(Z * 10 + 2);
      Exit;
    end;
    if (FromY > ToY) then
    begin
      if (FromX > ToX) then
        LoadBitmap(Z * 10 + 7) else LoadBitmap(Z * 10 + 1);
    end else begin
      if (FromX > ToX) then
        LoadBitmap(Z * 10 + 5) else LoadBitmap(Z * 10 + 3);
    end;   
  end;
...
begin
  ...
  if (Number > 0) then
    Graph.Surface.Canvas.Font.Color := cLtGreen
      else Graph.Surface.Canvas.Font.Color := cLtRed;
  FBack := Graphics.TBitmap.Create;
  ...
  try
    ...
    begin
      ...
      Graph.Surface.Assign(FBack);
      Graph.Surface.Canvas.TextOut(TX + DX, TY + TileSize + DY, IntToStr(Number));
      Graph.Render();
      Sleep(15);
    end;
  finally
