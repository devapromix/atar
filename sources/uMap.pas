...
uses Graphics, Classes, uCustomMap, uMiniMap, uItem, uScript, uTile, uDecorator, uMapGenerator;
...
uses SysUtils, Math, uAStar, uCreatures, uUtils, uBox, uError, uGraph, uColor,
  uLight, uResources;
...
begin
  ...
  with uCreatures.Creatures do begin
    L := Math.Max(Abs(AX - x), Abs(AY - y)) + 1;
    if(AX = X) and (AY < Y) then L := L * I;
    for I := 1 to L do
    begin
      LR := I / L;
      DX := AX + Trunc((X - AX) * LR);
      DY := AY + Trunc((Y - AY) * LR);
      VizCell(DX, DY, F);
      if not PC.FreeCell(DX, DY) and (I > 1) then Break;
    end;
  end;
end;
...
var
  ...
  and (Map.Cell[Y][X + 1].Tile in FloorSet) and (Map.Cell[Y + 1][X].Tile in FloorSet)
  and (Map.Cell[Y + 1][X + 1].Tile in FloorSet)
  ) or (
      (Map.Cell[Y][X - 1].Tile in FloorSet) and (Map.Cell[Y - 1][X].Tile = tlWall)
  and (Map.Cell[Y][X + 1].Tile = tlWall) and (Map.Cell[Y + 1][X].Tile in FloorSet)
  and (Map.Cell[Y + 1][X - 1].Tile in FloorSet)
  ) or (
      (Map.Cell[Y][X - 1].Tile in FloorSet) and (Map.Cell[Y - 1][X].Tile in FloorSet)
  and (Map.Cell[Y][X + 1].Tile = tlWall) and (Map.Cell[Y + 1][X].Tile = tlWall)
  and (Map.Cell[Y - 1][X - 1].Tile in FloorSet)
  ) or (
      (Map.Cell[Y][X - 1].Tile = tlWall) and (Map.Cell[Y - 1][X].Tile in FloorSet)
  and (Map.Cell[Y][X + 1].Tile in FloorSet) and (Map.Cell[Y + 1][X].Tile = tlWall)
  and (Map.Cell[Y - 1][X + 1].Tile in FloorSet)

  ) or ((Rand(1, 10) = 1) and (

    (((Map.Cell[Y][X - 1].Tile in ChestSet)
  and (Map.Cell[Y][X + 1].Tile in FloorSet) and (Map.Cell[Y + 1][X].Tile = tlWall))

  or ((Map.Cell[Y][X - 1].Tile in ChestSet)
  and (Map.Cell[Y][X + 1].Tile in FloorSet) and (Map.Cell[Y - 1][X].Tile = tlWall))
  ...
  or ((Map.Cell[Y - 1][X].Tile in ChestSet)
  and (Map.Cell[Y + 1][X].Tile in FloorSet) and (Map.Cell[Y][X + 1].Tile = tlWall))
...
begin
  try
    ...
    CreateMap(Self.Width, Self.Height, Info.GenID, DR, Self.Width * Self.Height); // супер 1, 7, 17, 18, 19, 21
    ...
    begin
      GenNewFloorPos();
      case Rand(1, 3) of
        1: Map.Cell[Y][X].Tile := tlLifeShrine;
        2: Map.Cell[Y][X].Tile := tlManaShrine;
        3: Map.Cell[Y][X].Tile := tlMegaShrine;  
      end;
    end;
    // Add chest
    ...
    begin
      GenChestPos();
      case Rand(1, 7) of
        1..3: Map.Cell[Y][X].Tile := tlClosedBarrel;
        4..5: Map.Cell[Y][X].Tile := tlClosedWoodChest;
        6: Map.Cell[Y][X].Tile := tlLockedWoodChest;
        7: Map.Cell[Y][X].Tile := tlLockedBestChest;
      end;
    end; 
    // Add decorators
    if not Info.Village then
    begin
      ...
      begin
        ...
        if not (Map.Cell[Y][X].Tile in FloorSet + [tlWall]) then Continue;
    ...
    if (Info.DecorType <> dtNone) then
      ...
      begin
        X := Rand(2, Map.Width - 3);
        Y := Rand(2, Map.Height - 3);
        SpotDraw(pdDecorator, X, Y, DecorTypeToStr(Info.DecorType), Info.DecTypSize);
      end;
    // Script
    // Map Level
    Level := Info.Level;
    ...
    on E: Exception do Error.Add('Map.Gen', E.Message);
  end;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      until (Cell[Y][X].Tile in FloorSet);
      SpotDraw(pdCreature, X, Y, V[Rand(0, High(V))], Clamp(Info.Level + 5, 5, 15))
    end;
  except
    on E: Exception do Error.Add('Map.AddCreatures', E.Message);
  end;
end;
    ...
    begin
      ...
      if (DungeonItems[uItem.Items.ItemIndex(S)].Category = dsGold) then
        SpotDraw(pdItem, X, Y, S, Clamp(Info.Level, 2, 5))
          else uItem.Items.Add(X, Y, GetRandItemID);
    end;
  except
    on E: Exception do Error.Add('Map.AddItems', E.Message);
  end;
end;
...
begin
  try
    ...
    on E: Exception do Error.Add('Map.Load', E.Message);
  end;
end;
...
begin
  try
    ...
    begin
      S := '';
      for X := 1 to Width do S := S + Tile[Cell[Y][X - 1].Tile].Tile;
      FM.Append(S);
    end;
    // Viz map
    FV.Clear;
    ...
    // Creatures
    with uCreatures.Creatures do if (Length(Enemy) > 0) then
    ...
    // Items
    with uItem.Items do if (Length(Item) > 0) then
    ...
    on E: Exception do Error.Add('Map.Save', E.Message);
  end;
end;
...
var
  ...
  X, Y, DX, DY: Integer;
begin
  for X := uCreatures.Creatures.PC.Pos.X - (Graph.RW + 1) to uCreatures.Creatures.PC.Pos.X + (Graph.RW + 1) do
  for Y := uCreatures.Creatures.PC.Pos.Y - (Graph.RH + 1) to uCreatures.Creatures.PC.Pos.Y + (Graph.RH + 1) do
  begin
    if (X < 0) or (Y < 0) or (X > MapSide - 1) or (Y > MapSide - 1) then Continue;
    Map.Cell[Y][X].FOV := False;
  end;
  ///
  for X := uCreatures.Creatures.PC.Pos.X - Graph.RW to uCreatures.Creatures.PC.Pos.X + Graph.RW do
  for Y := uCreatures.Creatures.PC.Pos.Y - Graph.RH to uCreatures.Creatures.PC.Pos.Y + Graph.RH do
  begin
    if (X < 0) or (Y < 0) or (X > MapSide - 1) or (Y > MapSide - 1) then Continue;
    if (GetDist(uCreatures.Creatures.PC.Pos.X, uCreatures.Creatures.PC.Pos.Y, X, Y) > uCreatures.Creatures.PC.GetRadius) then Continue;
    Map.LineFOV2(uCreatures.Creatures.PC.Pos.X, uCreatures.Creatures.PC.Pos.Y, x, y);
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        if not Map.Cell[Y][X].Viz then Continue;
        B := True;
      end;
      DX := (X - (uCreatures.Creatures.PC.Pos.X - Graph.RW)) * TileSize;
      DY := (Y - (uCreatures.Creatures.PC.Pos.Y - Graph.RH)) * TileSize + Graph.CharHeight;
      if not Map.Cell[Y, X].Viz then Map.VizCell(X, Y);
      case Map.Cell[Y][X].Tile of
        tlWall, tlHiddenDoor:
          if B then Draw(DX, DY, Res.FOGWALL) else Draw(DX, DY, Res.WALL);
        tlOpenDoor:
            if B then Draw(DX, DY, Res.FOGODOOR) else Draw(DX, DY, Res.ODOOR);
        tlClosedDoor..tlLockedDoor:
            if B then Draw(DX, DY, Res.FOGCDOOR) else Draw(DX, DY, Res.CDOOR);
        tlEmptyShrine..tlMegaShrine:
          begin
            if B then Draw(DX, DY, Res.SHRINE[Map.Cell[Y][X].Tile].FOG)
              else Draw(DX, DY, Res.SHRINE[Map.Cell[Y][X].Tile].IMG);
          end;
        tlOpenWoodChest..tlClosedBarrel:
          begin
            if B then Draw(DX, DY, Res.CHEST[Map.Cell[Y][X].Tile].FOG)
              else Draw(DX, DY, Res.CHEST[Map.Cell[Y][X].Tile].IMG);
          end;
        tlStone:
            if B then Draw(DX, DY, Res.FOGSTONE) else Draw(DX, DY, Res.STONE);
        tlGrass:
            if B then Draw(DX, DY, Res.FOGGRASS) else Draw(DX, DY, Res.GRASS);
        tlTree:
            if B then Draw(DX, DY, Res.FOGTREE) else Draw(DX, DY, Res.TREE);
        tlBush:
            if B then Draw(DX, DY, Res.FOGBUSH) else Draw(DX, DY, Res.BUSH);
        tlPrevDungeon:
            if B then Draw(DX, DY, Res.FOGUP) else Draw(DX, DY, Res.UP);
        tlNextDungeon, tlAltNextDungeon:
            if B then Draw(DX, DY, Res.FOGDOWN) else Draw(DX, DY, Res.DOWN);
        else case Map.Info.FloorTile of
          tlGrass:
            if B then Draw(DX, DY, Res.FOGGRASS) else Draw(DX, DY, Res.GRASS);
          tlFloor:
            if B then Draw(DX, DY, Res.FOGFLOOR) else Draw(DX, DY, Res.FLOOR);
        end;
      end;
      if B then Continue;
      case Map.Cell[Y][X].Tile of
      ...
      if not ParamLight then Light.Render(X, Y, DX, DY);

      // Look
      if (CursorMode <> cmNone) then
      if ((uCreatures.Creatures.PC.Look.X = X) and (uCreatures.Creatures.PC.Look.Y = Y)) then
      begin
        Brush.Style := bsClear;
        case CursorMode of
          cmLook : Pen.Color := cLtYellow;
          cmShoot: Pen.Color := cLtRed;
        end;
