...
uses Windows, Graphics, SysUtils, Types;
...
function Explode(const Separator: Char; Source: string): TExplodeResult; overload;
function Explode(const Count: Integer; Source: string): TExplodeResult; overload;
...
uses Math, Forms, uItem, uLang, uTile, uMap, uLog, uCreatures, Classes, uAStar,
  uError;
...
begin
  Result := Value;
  if (Result < AMin) then Result := AMax;
...
function LastPos(const SubStr, Source: string; const IgnoreCase: Boolean): Integer;
var
...
function Percent(N, P: Integer): Integer;
begin
  if (P = 0) then Result := N else Result := N * P div 100
end;
...
begin
  ...
  begin
    ...
    if not Creatures.PC.FreeCell(AX, AY) then Exit;
  end;
  Result := True;
{
...
begin
  ...
  try
    ...
    on E: Exception do Error.Add('Utils.DelFile', E.Message);
  end;
end;

procedure TransKeys(var Key: Word);
begin
  if (Key = 100) then Key := 37;
  if (Key = 104) then Key := 38;
  if (Key = 102) then Key := 39;
  if (Key =  98) then Key := 40;
end;
...
begin
  ...
  case Map.Cell[Creatures.PC.Pos.Y][Creatures.PC.Pos.X].Tile of
    ...
    begin
      Log.Add(GetLang(45));
      Map.Cell[Creatures.PC.Pos.Y][Creatures.PC.Pos.X].Tile := tlOpenWoodChest;
      J := 3;
    end;
    tlLockedBestChest:
    begin
      Log.Add(GetLang(45));
      Map.Cell[Creatures.PC.Pos.Y][Creatures.PC.Pos.X].Tile := tlOpenBestChest;
      J := 5;
    end;
  end;
  for I := 0 to J do
  begin
    ...
    if not F or (Rand(1, 2) = 1) then Break;
  end;
end;
