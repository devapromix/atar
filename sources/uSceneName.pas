...
uses Classes, uScene, uSceneBaseMenu;
...
uses SysUtils, uCreatures, uScenes, uSceneGame, Graphics, uGraph, uColor,
  uGame, uMain, uSceneMenu, uName, uUtils, uBox, uError, uLang, uSceneRace;
...
begin
  ...
  try
    ...
    on E: Exception do Error.Add('SceneName.KeyDown (#'
      + IntToStr(Key) + ')', E.Message);
  end;
end;       
...
begin
  ...
  try
    ...
    begin
      ...
      Font.Style := [];
      H := TextWidth(S);
      TextOut((Graph.Width div 2) - (W div 2),
        (Graph.Height div 2) - (Graph.CharHeight div 2), S);
      Font.Style := [fsBold];
      Font.Color := cAcColor;
      S := #32 + Creatures.PC.Name + '_';
      TextOut((Graph.Width div 2) - (W div 2) + H,
        (Graph.Height div 2) - (Graph.CharHeight div 2), S);
      if (Creatures.PC.Name = '') then N := 'enter, ' + N;
      Graph.Text.BarOut(N, 48);
      if FileExists(Path + 'save\' + Creatures.PC.Name + '.sav') then
      begin
        P := Game.GetPCInfo(Path + 'save\' + Creatures.PC.Name + '.sav');
        S := GetLang(30) + ': ' + IntToStr(P.Level) + ' | ' + GetLang(36) + ': ' + IntToStr(P.Rating)
          + ' | ' + GetLang(140) + ': ' + GetFileDate(Path + 'save\' + Creatures.PC.Name + '.sav');
        Font.Color := cBgColor;
        Graph.Text.TextCenter(((Graph.Height div 2) div Graph.CharHeight) + 2, AnsiLowerCase(S));
        Graph.Text.BarOut('enter', 1);
      end;
    end;
    Graph.Render;
  except
    on E: Exception do Error.Add('SceneName.Render', E.Message);
  end;
end;
