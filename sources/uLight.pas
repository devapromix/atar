...
uses Graphics, SysUtils, Types, uUtils, uGraph, uMap, uCreatures, uDecorator,
     uTile;
...
var
  ...
  D, H: Double;
begin
  if (Creatures.PC.Pos.X = X) and (Creatures.PC.Pos.Y = Y) then Exit;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (D < Max - AR) then D := Max - AR;
      if (D < Round(LightMin * 10)) then D := Round(LightMin * 10);
