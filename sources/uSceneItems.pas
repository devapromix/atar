...
uses Classes, Graphics, Windows, uScene, uSceneBaseGame;
...
uses SysUtils, uSceneGame, uScenes, uError, uGraph, uItem, uCreatures,
  uColor, uUtils, uBox, uLang, uSceneInv;
...
begin
  ...
  try
    case Key of
      ...
      begin
        I := (Key - (ord('A'))) + 1;
        if (I <= Items.CellItemsCount(Creatures.PC.Pos.X, Creatures.PC.Pos.Y)) then
        begin
          Items.Pickup(I);
          if (Items.CellItemsCount(Creatures.PC.Pos.X, Creatures.PC.Pos.Y) = 0) then
            Scenes.Scene := SceneGame else Render;
        end;
      end;
    end;
  except
    on E: Exception do Error.Add('SceneItems.KeyDown (#' + IntToStr(Key) + ')', E.Message);
  end;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      if (C > 0) and (Length(Items.Item) > 0) then
        for I := 0 to High(Items.Item) do
          if (Items.Item[I].Pos.X = Creatures.PC.Pos.X) and (Items.Item[I].Pos.Y = Creatures.PC.Pos.Y) then
          begin
            ...
            Items.SetColor(V);
            if ((DungeonItems[V].MaxTough > 0) and (Items.Item[I].Prop.Tough <= 0)) then Font.Color := cRed;
            Graph.Text.DrawOut(5, Y, GetItemLang(DungeonItems[V].Sprite) + Items.GetItemProp(Items.Item[I].Count,
              Items.Item[I].Prop.Tough, I, V) + Items.GetWeight(V));
            Inc(Y);
          end;
      Items.RenderPCInvStat(Y);
      if (C = 1) then Graph.Text.BarOut('enter, a', GetLang(26), False)
        else if (C > 1) then
    ...
    on E: Exception do Error.Add('SceneItems.Render', E.Message);
  end;
end;
