...
uses SysUtils, uUtils, uError, uMap, uGraph, uCreatures, uScenes, uLog, uBox,
  uSceneItem, uTile, uSceneItems, uLang, uTempSys, uInv, uSkill, uFormulas;
...
begin
  ...
  case DungeonItems[V].Category of
    dsLHand : S := GetLang('on left hand',  'в левой руке' );
    dsRHand : S := GetLang('on right hand', 'в правой руке');
    dsBody  : S := GetLang('on body',       'на торсе'     );
    dsHead  : S := GetLang('on head',       'на голове'    );
    dsFoot  : S := GetLang('on feet',       'на ногах'     );
    dsRing  : S := GetLang('on finger',     'на пальце'    );
    dsAmulet: S := GetLang('on neck',       'на шее'       );
         else S := '';
  end;
