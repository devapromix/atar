...
uses uUtils, uGraph, uColor, uLight, uMap, uDecorator;
...
var
  ...
  procedure SetTile(var AImg, AFog: Graphics.TBitmap; ResName, S: string; IsTransparent: Boolean = False);
  begin
    if (S = '') then S := ResName;
    ...
    if not ParamLight then Gamma(AFog, LightMin);
  end;
...
begin        
  ...
  case Map.Info.FloorTile of
    tlGrass: F.Assign(GRASS);
    tlFloor: F.Assign(FLOOR);
  end;
  ...
  begin
    ...
    begin
      ...
      if not ParamLight then Gamma(SHRINE[I].FOG, LightMin);
    end;
    ...
    begin
      ...
      if not ParamLight then Gamma(CHEST[I].FOG, LightMin);
    end;
  end;
  // Lock
  ...
  if not ParamLight then Gamma(FOGCDOOR, LightMin);
  ...
  if not ParamLight then Gamma(FOGODOOR, LightMin);
