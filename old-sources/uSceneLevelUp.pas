...
uses Graphics, SysUtils, uGraph, uCreatures, uScenes, uSceneGame, uColor, uBox,
  uError, uLog, uLang, uUtils;
...
begin
  try
    ...
    begin
      case I of
        0: S := S + #32 + IntToStr(Creatures.PC.Prop.Strength);
        1: S := S + #32 + IntToStr(Creatures.PC.Prop.Dexterity);
        2: S := S + #32 + IntToStr(Creatures.PC.Prop.Will);
        3: S := S + #32 + IntToStr(Creatures.PC.Prop.Speed);
      end;
      if (CursorPos = P) then
      begin
        Font.Color := cAcColor;
      ...
      Font.Color := cLtBlue;
      if (P = CursorPos) then TextOut((Graph.Width div 2) -
        (TextWidth(S) div 2) + ((Length(S) + 1) * Graph.CharWidth),
        (P * Graph.CharHeight) + T + Graph.CharHeight, '+ 1');
      Inc(P);
    end;
  except
    on E: Exception do Error.Add('SceneLevelUp.AtrItem', E.Message);
  end;
end;
...
begin
  try
    ...
    on E: Exception do Error.Add('SceneLevelUp.KeyDown (#' + IntToStr(Key) + ')', E.Message);
  end;
end;
...
begin
  ...
  try
    ...
    with Graph.Surface.Canvas do
    begin
      for I := 0 to Count - 1 do AtrItem(I, GetLang(I + 15));
    ...
    on E: Exception do Error.Add('SceneLevelUp.Render', E.Message);
  end;
end;
