...
function DoAStar(MapX, MapY, FromX, FromY, ToX, ToY: Integer; Callback: TGetXYVal;
...
var
  ...
  begin
    if not InRange(X, 1, MapX) or not InRange(Y, 1, MapY) then Exit;
    with Cells[X, Y] do
    begin
...
begin
  Result := False;
  if not InRange(ToX, 1, MapX) or not InRange(ToY, 1, MapY) then Exit;
  NOpen := 0;
